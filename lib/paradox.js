...
module.exports = class ParadoxConnector {
    ...
    TranslateCommand(command) {
        ...
        if (command.indexOf("AL") === 0) {
            ...
            var name = +command.substring(6, command.length).trim();
            this.SendMQTTEvent("homeassistant/alarm_control_panel/" + this._config.name + "/" + name + "/config", {
                name: name,
                code_arm_required: true, //TODO Variable
                code_disarm_required: true, //TODO Variable
                code: this._config.panelUserCode,
                object_id: this._config.name + "_" + partition,
                device: device
            }) //TODO
        } else {
